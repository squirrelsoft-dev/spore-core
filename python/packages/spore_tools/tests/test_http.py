...
async def test_http_get_returns_body(httpx_mock: HTTPXMock) -> None:
    ...
    sb = AllowAllSandbox()
    r = await HttpGetTool().execute(_call("http_get", {"url": "https://example.test/hello"}), sb)
...
async def test_http_get_invalid_url_is_recoverable() -> None:
    sb = AllowAllSandbox()
    r = await HttpGetTool().execute(_call("http_get", {"url": "not-a-url://////"}), sb)
...
async def test_http_invalid_params_recoverable(tool: type) -> None:
    sb = AllowAllSandbox()
    r = await tool().execute(_call(tool.NAME, {}), sb)
    assert isinstance(r, ToolOutputError)
    assert r.recoverable is True
