...
> **ANCHOR LANDED (Rust) `ca41f8f` (2026-06-24).** The four turn/tool call sites (`fire_before_turn`/`fire_before_tool`/`fire_before_completion`/`fire_after_tool`) now consult the rich priority-ordered chain; the harness-local `MiddlewareChain`/`MiddlewareDecision`/4-variant `HookPoint` stubs are deleted (public names re-export the rich `middleware::` types). **SC-9** lands via a new `ContextManager::replace_tool_result` seam (loop tracks each result's `session_state.messages` index 1:1 and re-renders on `ContinueWithModification`); **SC-11** via in-place `&mut calls`. The BeforeCompletion **`ForceAnotherTurn`** injection (absent from the stub) is also wired. Additive — no-middleware path byte-identical, no fixture/wire impact. **Deferred:** `BeforeSession`/`AfterSession` are still unwired (the stub never fired them either; needs a single-exit `run_react` refactor), and the loop emits no `MiddlewareSpan`s. Tests: SC-9/SC-11/ForceAnotherTurn acceptance + adapter unit; suite green (1242 lib pass). TS/Py/Go parity **#158**. **Remaining Phase 3:** SC-10, SC-26/#115, SC-28 (below).
...
**SC-26 — Rich `ContextSources` not wired into the live loop (#115). Surfaced via cordyceps (skills); affects guides/memory/prompt-chunks. — GUIDES + SKILLS DONE (Rust) `a061b6d`/`18ed309`/`00b6106`/`2a9b62b`; memory deferred.** `SkillInjectingContextManager` forwarded 9 methods + injected skills as User messages. Root cause: the harness-loop seam `ContextManager::assemble(session, task)` carried **no `ContextSources`**, so the structural injection was bypassed (#115). *Fix (landed):* `assemble` now takes `sources: &ContextSources`; the harness builds them per turn; the production context manager renders composed-prompt + guides + (memory) into a **leading System block** (the loop merges the system prompt in front), and skills are baked into the harness — a core `SkillCatalog` (+ `load_skill` tool) feeds `active_guides()` into `ContextSources.guides`. *Acceptance:* a consumer registers skills **and** a guide source; all reach the model via the structural seam, no wrapper; cordyceps **deleted `SkillInjectingContextManager`** (live-verified: the model called `load_skill` + followed the procedure with no shim). ✅ for guides + skills. **Memory DEFERRED** — `Arc<dyn MemoryProvider>` needs an SC-2-class object-safety conversion first (RPITIT trait); `ContextSources.memory` stays empty, render path already accepts it. Tracked **#160**; TS/Py/Go parity **#159**. (The `#32` cache-hash halt machinery stays dormant — that's still #7's job.)
