...
- *Looper inertness:* SC-30 lands green but **inert for looper until SC-BUG-1** — the reviewer (and looper's gates-based `/verify`) run in the eval frame, the frame HITL-resume discards. SC-BUG-1 is the real unlock.
