...
- A `MarkdownMemoryProvider` — a concrete `MemoryStore` that reads/writes a
  `.md` file (`src/memory_provider.rs`).
...
- The `StorageProvider`/`MemoryStore` trait as the clean swap point.
...
let harness = HarnessBuilder::conversational(model)
    .storage(Arc::new(storage))      // ← the seam
...
`ctx.session_id()`. So **both phases must use the same id** or recall reads an
empty session. This example pins:

```rust
fn session() -> SessionId {
    SessionId::new("project-ironwood")   // NOT SessionId::generate()
}
```

`SessionId::generate()` would give Run 2 a different key and it would recall
nothing. The provider also stamps the session id into each `memory.md` block,
so one file can hold several sessions without cross-talk.
...
swallowed into an entry. (See the unit tests in `memory_provider.rs`.)
...
cd examples/rust/07-memory
cargo run -- --phase store     # agent writes facts → memory.md
cat memory.md                  # inspect the human-readable artifact
cargo run -- --phase recall    # agent answers from memory.md alone
```

Running with **no flag** runs `store` and then points you at `recall`:

```sh
cargo run                      # = --phase store, then prints the next step
```
