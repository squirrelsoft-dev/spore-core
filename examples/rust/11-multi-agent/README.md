...
[`SubagentTool`], which implements the same `Tool` trait as `write_file` or
...
example 06 — build a `StandardTool` from the boxed `SubagentTool` plus a schema,
then `.tool(...)` it onto the orchestrator's builder. The only difference is that
the "implementation" boxed inside is a whole agent.

## Independent agents, isolated context

Both workers are wrapped with [`ContextSharing::Isolated`]:
...
- Each **worker** runs **ReAct** internally. This is hardcoded inside
  `SubagentTool`: a subagent always runs its child as `ReAct { max_iterations }`.
...
cargo run                                   # default model + topic
cargo run -- --topic "the history of the TCP/IP protocol suite"
...
> **not** part of the build/clippy/fmt gate, which is all CI checks.

## Where to look

- `src/main.rs` — the whole example. Start at `main`; the agent-as-tool wiring is
  in `build_research_harness`, `build_writing_harness`, and `build_worker_tool`.
- `rust/crates/spore-core/src/tools/subagent.rs` — `SubagentTool`: how a child
  `Harness` is exposed as a `Tool`, the `instruction` input, and the depth-1
  (no nested subagents) rule.
- `examples/rust/06-web-research/` — the research worker's `web_search` loop, run
  standalone.
- `examples/rust/08-plan-execute/` — `LoopStrategy::PlanExecute` on its own.

[`SubagentTool`]: ../../../rust/crates/spore-core/src/tools/subagent.rs
[`ContextSharing::Isolated`]: ../../../rust/crates/spore-core/src/tools/subagent.rs
