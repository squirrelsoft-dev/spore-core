...
// TestRegistryValidates is the regression guard that the orchestrator's post-#119
// composed PlanExecute(plan: ReAct{plan-schema}, execute: ReAct) tree stays
...
func TestRegistryValidates(t *testing.T) {
	...
	registry := sporecore.NewExecutionRegistryBuilder().
		...
		Build()
	task := sporecore.NewTask("orchestrate three agents", sporecore.NewSessionID(), planExecuteStrategy())
