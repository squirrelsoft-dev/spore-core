...
- **Registering custom tools.** Each tool implements the `sporecore.Tool`
  interface (`Name`, `IsSubagentTool`, `MayProduceLargeOutput`, `Execute`) and is
  registered on a `*sporecore.StandardToolRegistry` with a `RegistryToolSchema`.
  In Go, `StandardToolRegistry` IS the harness-loop tool registry, so a populated
  one plugs straight into the harness config. Because the tools are pure functions
  (no filesystem), the conversational defaults — including `NullSandbox` — are
  fine; we only override the registry.
- **The loop, made visible.** The program prints each turn (`think · turn N`, via
  the `OnStream` sink keyed on `HarnessStreamTurnStart`) and each tool call +
  result (`act → name(args) = result`, from inside `Execute`), so you can watch
  the agent work:
