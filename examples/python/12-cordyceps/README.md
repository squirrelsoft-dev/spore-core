...
The orchestrator runs `LoopStrategyReAct`, not `PlanExecute`. The PlanExecute
...
endpoint — only the model id differs (`OllamaModelInterface.with_base_url`).
...
mid-loop with a `RunResultConsult`, and the `analysis_worker`'s `SubagentTool`
...
runs it, and resumes the worker with the answer. The orchestrator's model is
never involved in a consult; mediation is deterministic.
...
two custom tools, each lowering to `ToolOutputConsult.consult(ConsultRequest(kind, …))`:
...
`with_consult_handlers` (not on the builder — mediation fires at the tool).
...
1. **Best-practices research** (`research`, budget 5). Looking up an idiom is
   *normal*, not distress, so it never reaches the human. On the 6th consult the
   `SoftFail` policy resumes the worker with `ConsultResponseBudgetExhausted` and
   it finishes on general knowledge.
2. **Advisor** (`advice`, budget 3). For *is-this-real / how-bad-is-this*
   ...
   consult into `RunResultWaitingForHuman`, which bubbles up to the REPL with
   three choices:
...
resume through the parent** — `_resume_inner` currently no-ops the `child_state`
branch (it lands with the #5/#115 follow-up). So in this example every human
...
harness budget-bump). This is a faithful demonstration of the *ladder shape*;
the lossless child-resume is a tracked follow-up, not something this example
hacks into core.
...
assembles each turn via `StandardCompactionAdapter.assemble`, a pass-through of
`session.messages`; the rich `assemble` that would inject `pending_skill_injections`
(plus chunks and merged memory) is bypassed pending the deferred #7 ContextManager
migration (cf. Known Deviation #8). So today a skill can reach the model only as a
...
(`HarnessBuilder.guide_registry(..)` + a standard `load_skill` tool + sticky
...
### How it works here (three pieces, in `skills.py` + `tools/`)

1. **`SkillCatalog`** (`skills.py`) scans `.spore/skills/{name}/SKILL.md`
   ...
   **manifest side-list** of `(name, description)` — `select` does not surface a
   guide's description, so the example owns the manifest text. The bundled
   `skills/audit/SKILL.md` is read at startup and always registered, so the
   example is self-contained even with an empty `.spore/skills/`; on first run it
   also **seeds** `.spore/skills/audit/SKILL.md` from that bundled copy so you can
   see the filesystem-registry shape.
2. **`load_skill` tool** (`tools/load_skill.py`, closes over the registry):
   confirms the named skill exists, then appends its id to
   `run_store["active_skills"]` (deduped) via the tool context's run store. No
   new `ToolOutput` variant — this is the storage-backed flavor (#115 "flavor B").
3. **`SkillInjectingContextManager`** (`skills.py`) wraps the standard compaction
   adapter and, in `assemble` only, prepends — **ephemerally**, never into
   `session.messages` — (a) a **manifest** of every skill (`name: description`,
   progressive disclosure), and (b) the **full body** of every id in
   `run_store["active_skills"]`. Every other context-manager method delegates
...
loaded procedure. A unit test in `test_skills.py`
(`test_manifest_always_injected_bodies_only_when_active`) asserts exactly this:
manifest always present, body present only after `load_skill`.
...
```sh
uv run pytest          # unit tests (skill injection + frontmatter parsing)
uvx ruff check .       # lint
