...
| Sandbox | default null sandbox (pure tools)                        | `WorkspaceScopedSandbox` over `sample-files/`          |
...
03's tools were pure functions, so the `conversational(model)` default null
sandbox was fine. Catalogue **file** tools operate *through* a sandbox, so 04
...
harness **stream events** (`StreamToolCall` carries the name + args,
`StreamToolResult` carries the result content) rather than from inside a
...
```sh
cd examples/python/04-filesystem-agent
uv run main.py
