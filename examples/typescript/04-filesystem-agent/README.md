...
| Output  | stream-printed `think` / `act`                      | stream-printed `think` / `act` / `obs` _(same)_ |
| Tools   | hand-rolled `ToolRegistry` (`schemas` + `dispatch`) | `.tools(StandardTools.codingSet())`             |
...
harness **stream events** (`tool_call` carries the name + args, `tool_result`
...
```sh
cd examples/typescript/04-filesystem-agent
pnpm install
pnpm start
pnpm start -- --prompt "List the files and tell me which one mentions nutrients."
```
