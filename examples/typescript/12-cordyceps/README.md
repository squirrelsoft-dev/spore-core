...
This is the capstone of the TypeScript suite and the "very basic" seed of the
...
The orchestrator runs `{ kind: "re_act" }`, not `plan_execute`. The PlanExecute
...
Each `analysis_worker` audits exactly **one** module and runs `{ kind:
"isolated" }`: a fresh session with no shared mutable state. The worker may burn
a dozen turns grepping and reading narrow line ranges, but only its final JSON
findings cross back into the orchestrator's context. This keeps each worker
inside its own context window and keeps the orchestrator lean across a long,
many-module audit. Findings accumulate in the `memory` tool, which survives
compaction — so a long run does not lose earlier modules' findings.
...
endpoint — only the model id differs (`OllamaModelInterface.withBaseUrl`).

### Depth-1 → orchestrator-mediated consult

Subagents cannot spawn subagents (the type system enforces depth-1). So when the
`analysis_worker` needs help, it does **not** spawn a helper itself. It pauses
mid-loop with a `RunResult` `consult`, and the `analysis_worker`'s `SubagentTool`
— sitting at the orchestrator boundary — **mediates** the consult: it routes by
`kind` to the right handler (which is the *orchestrator's* direct child, depth-1),
runs it, and resumes the worker with the answer via `harness.resumeConsult`. The
orchestrator's model is never involved in a consult; mediation is deterministic.
...
two custom tools, each lowering to `toolOutput.consult({ kind, … })`:

| Tool | `kind` | Routed to | Budget | Overflow |
| --- | --- | --- | --- | --- |
| `research_best_practices` | `research` | research_worker (web_search) | **5** | `soft_fail` |
| `consult_advisor` | `advice` | advisor (cloud model) | **3** | `escalate_to_human` |

Both handlers are installed on the `analysis_worker` `SubagentTool` via its
`consultHandlers` map (passed in `SubagentTool.buildOrThrow({ …, consultHandlers
})`). The TS mediator reads that map at the tool boundary — there is no separate
`withConsultHandlers` call as in the Rust reference; the handlers are part of the
subagent config. (Mediation still fires at the tool, never on the builder.)
...
1. **Best-practices research** (`research`, budget 5). Looking up an idiom is
   *normal*, not distress, so it never reaches the human. On the 6th consult the
   `soft_fail` policy resumes the worker with `ConsultResponse` `budget_exhausted`
   and it finishes on general knowledge.
2. **Advisor** (`advice`, budget 3). For *is-this-real / how-bad-is-this*
   questions. On the 4th consult, `escalate_to_human` converts the over-budget
   consult into `RunResult` `waiting_for_human`, which bubbles up to the REPL with
   three choices:
...
The worker's paused consult lives inside the orchestrator's
`PausedState.child_state`, and the harness does **not yet wire a child-consult
resume through the parent** — the same #114 limitation the Rust reference
documents (it lands with the #5/#115 follow-up). So in this example every human
choice resumes the **orchestrator** (via `harness.resume(state, response)`) with
your decision injected as guidance, and the specific module's in-flight worker
audit is dropped. **"+1 advisor turn"** re-runs the advisor handler **host-side**
(a fresh `advisorHandler.run(...)`) and injects its answer as that guidance — the
closest we can get to a "budget bump" without a core primitive (there is no
...
into core. The TS `resume` API behaves exactly like the Rust one here: it resumes
the orchestrator, not the worker consult.
...
`GuideRegistry → pending_skill_injections` (the rich `StandardContextManager`'s
Block-3 segment) — **isn't live-wired in the harness loop yet**. The live loop
assembles each turn via `StandardCompactionAdapter`'s `assemble`, a pass-through
of `session.messages`; the rich `assemble` that would inject
`pending_skill_injections` (plus chunks and merged memory) is bypassed pending
the deferred #7 ContextManager migration (cf. Known Deviation #8). So today a
skill can reach the model only as a tool-result message or via a **custom context
manager**.

This example takes the custom-context-manager route and documents it honestly,
because it is exactly the pattern **[#115](https://github.com/squirrelsoft-dev/spore-core/issues/115)**
will absorb into the library (`HarnessBuilder` skill-registry wiring + a standard
`load_skill` tool + sticky injection, all four languages). The companion
productionization — a scope-aware `FileSystemGuideRegistry`/`CompositeGuideRegistry`
over `.spore/skills/` (the direct sibling of
**[#88](https://github.com/squirrelsoft-dev/spore-core/issues/88)**'s
`FileSystemChunkProvider`) — is also tracked in #115; this example inlines the
filesystem scan.

### How it works here (three pieces, in `src/skills.ts` + `src/tools/`)

1. **`SkillCatalog`** (`src/skills.ts`) scans `.spore/skills/{name}/SKILL.md`
   ...
   each as a `skill`-type `Guide` in a `StandardGuideRegistry`. It also keeps a
   **manifest side-list** of `(name, description)` — `Guide` has no `description`
   field, so the example owns the manifest text. The bundled
   `skills/audit/SKILL.md` is read from disk at startup (the TS analogue of
   Rust's `include_str!`) and always registered, so the example is self-contained
   even with an empty `.spore/skills/`; on first run it also **seeds**
   `.spore/skills/audit/SKILL.md` from that bundled copy so you can see the
   filesystem-registry shape.
2. **`load_skill` tool** (`src/tools/load-skill.ts`, closes over the registry):
   confirms the named skill exists, then appends its id to
   `runStore["active_skills"]` (deduped, keyed by the worker's `sessionId`). No
   new `ToolOutput` variant — this is the storage-backed flavor (#115 "flavor B").
3. **`SkillInjectingContextManager`** (`src/skills.ts`) implements the
   harness-loop `ContextManager` interface, wrapping the standard compaction
   adapter. In `assemble` only, it prepends — **ephemerally**, never into
   `session.messages` — (a) a **manifest** of every skill (`name: description`,
   progressive disclosure), and (b) the **full body** of every id in
   `runStore["active_skills"]`. Every other method delegates verbatim to the
   inner adapter (forwarding the optional compaction methods only when the inner
...
until the session is cleared. Because the active set lives in `runStore` (not the
message history), it is **compaction-proof** — a long audit cannot "forget" the
loaded procedure. A unit test in `tests/skills.test.ts`
(`manifest_always_injected_bodies_only_when_active`, mirroring the Rust
reference's test of the same name) asserts exactly this: manifest always present,
body present only after `load_skill`.
...
The skill text is language-agnostic and identical to the Rust reference's.

## REPL flow

1. Opens with the **pre-filled audit prompt** (from the issue). Press **enter**
   to accept it verbatim, or type your own.
...
| `--search-url` / `SPORE_WEB_SEARCH_ENDPOINT` | `http://localhost:8888/search?format=json` | research worker's `web_search` |
...
```sh
pnpm test       # vitest: skill-injection + frontmatter-parsing unit tests
pnpm lint       # eslint
